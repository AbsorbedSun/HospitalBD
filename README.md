...
Copiar el archivo de ejemplo y editarlo con los datos de tu SQL Server:
...
# O directamente con Python (cualquier SO)
...
> ✅ **Un solo proceso sirve tanto la API (`/api/*`) como el frontend.**
> Ya no hay dos servidores ni dos puertos — todo en `http://127.0.0.1:5000`.
...
- Abrir la aplicación desde `http://127.0.0.1:5000` (no desde `file://`)
- Verificar que `python run.py` esté corriendo
...
- Con el servidor unificado (`python run.py`) no debería haber errores CORS — frontend y API están en el mismo origen
- Si usas Live Server de VS Code para desarrollo del frontend, el puerto 5500 ya está permitido en la configuración CORS de `app.py`
